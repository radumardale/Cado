    ...
    <div className='mt-12 lg:mt-0'>
      {product.nr_of_items > 1 && (
        ...
        >
          ...
          <p className='mb-6 mt-4 font-bold text-sm lg:text-base leading-4 lg:leading-5'>
            *{prod_t('includes_info')}{' '}
            <a href='mailto:order@cado.md' className='underline'>
              order@cado.md
            </a>
          </p>
        </Accordion>
      ...
      <Accordion
        ...
        open={activeIndex == 2}
        setActiveIndex={() => {
          setActiveIndex(activeIndex === 2 ? -1 : 2);
        }}
        title={prod_t('features')}
      >
        {product.optional_info && product.optional_info.material[locale] !== '' && (
          <div className='flex gap-x-1 not-first:mt-4'>
            <p className='text-sm lg:text-base leading-4 lg:leading-5 font-semibold'>
              {prod_t('material')}:
            </p>
            <p className='text-black text-sm lg:text-base leading-4 lg:leading-5'>
              {product.optional_info.material[locale]}
            </p>
          </div>
        )}
        ...
        <div className='flex gap-x-1 not-first:mt-4'>
          ...
          {product.categories.map((category, index) => {
            return (
              <p key={index} className='text-black text-sm lg:text-base leading-4 lg:leading-5'>
                {t(`Tags.${category}.title`)}
        ...
        <div className='flex gap-x-1 flex-wrap mt-4'>
          ...
          {product.product_content.map((product_content, index) => {
            return (
              <p key={index} className='text-black text-sm lg:text-base leading-4 lg:leading-5'>
                {t(`product_content.${product_content}`)}{' '}
                {index < product.product_content.length - 1 && ','}
              </p>
            );
          })}
        </div>
        <div className='flex gap-x-1 flex-wrap mt-4'>
          ...
          {product.ocasions.map((ocasions, index) => {
            return (
              <p key={index} className='text-black text-sm lg:text-base leading-4 lg:leading-5'>
                {t(`ocasions.${ocasions}.title`)} {index < product.ocasions.length - 1 && ','}
              </p>
            );
          })}
        </div>
        {product.nr_of_items > 1 && (
          <p className='text-sm lg:text-base leading-4 lg:leading-5 mt-4 font-semibold'>
            {prod_t('number')}: {product.nr_of_items}
          </p>
        )}
        <p className='text-sm lg:text-base leading-4 lg:leading-5 mt-4 font-semibold'>
          {prod_t('id')}: <span className='font-normal'>{product.custom_id}</span>
        </p>
      </Accordion>
    </div>
  );
