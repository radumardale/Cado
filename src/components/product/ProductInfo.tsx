...
export default function ProductInfo({ id }: ProductInfoInterface) {
  ...
  return (
    <>
      <Header
        category={categoryParam ? categoryParam : data?.product?.categories[0]}
        breadcrumbs
        productInfo={{
          id: data?.product?.custom_id || '',
          title: data?.product?.title[locale] || '',
        }}
      />
        ...
        {!data?.product ? (
          <>
            <div className='col-span-full lg:col-start-2 lg:col-span-5 grid grid-cols-5 lg:grid-cols-5 mt-2 lg:mt-16 gap-x-6 mb-4 lg:mb-31 relative h-fit cursor-pointer -ml-4 w-[calc(100%+2rem)] lg:mx-0 lg:w-full px-4 lg:px-0 overflow-hidden lg:overflow-visible'>
              ...
              <div className={`h-auto w-full col-span-full relative group flex gap-x-6`}>
      ...
      {data?.product?.categories[0] && (
        <SimiliarProducts
          category={categoryParam || data?.product?.categories[0]}
          productId={data.product.custom_id}
        />
      )}
    </>
  );
