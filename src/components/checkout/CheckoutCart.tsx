...
export default function CheckoutCart({
  ...
  const [deliveryPrice, setDeliveryPrice] = useState(
    deliveryRegion ? getDeliveryPrice(deliveryRegion) : null
  );
    ...
    if (products.length > 0)
      setTotalCost(
        items.reduce((acc, item) => {
          ...
          return (
            acc +
            (product && product.sale.active ? product.sale.sale_price : product?.price || 0) *
              item.quantity
          );
        }, 0) +
          (deliveryPrice
      ...
      {items.length > 0 && mounted && products.length > 0 ? (
        ...
        >
          {items.map((item, index) => {
            const product =
              products.find((product: ProductInterface) => product.custom_id === item.productId) ||
              undefined;
            if (!product) return;
                    ...
                    items
                      .reduce((acc, item) => {
                        const product = products.find(
                          product => product.custom_id === item.productId
                        );
                        return (
                          acc +
