...
export default function ContactForm() {
  ...
  return (
    <>
      <h3 className='col-span-full lg:text-center mb-8 lg:mb-16 font-manrope text-2xl lg:text-3xl leading-7 lg:leading-11 uppercase font-semibold'>
        {t('title')}
      </h3>
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit(onSubmit)}
          className='col-span-full lg:col-start-4 lg:col-span-9 grid grid-cols-8 lg:grid-cols-9'
        >
                ...
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange}>
                    ...
                    <SelectContent className='border-gray'>
                      <SelectGroup>
                        <SelectItem className='text-base cursor-pointer  ' value={'ORDER_ISSUE'}>
                          {t('order_issue')}
                        </SelectItem>
                        <SelectItem className='text-base cursor-pointer  ' value={'GIFT_ASSITANCE'}>
                          {t('gift_assistance')}
                        </SelectItem>
                        <SelectItem
                          className='text-base cursor-pointer  '
                          value={'COMPANY_COLLABORATION'}
                        >
                          {t('company_collaboration')}
                        </SelectItem>
                        <SelectItem className='text-base cursor-pointer  ' value={'OTHER'}>
                          {t('other')}
                        </SelectItem>
                      </SelectGroup>
                    </SelectContent>
              ...
              render={({ field }) => (
                <FormItem className='text-base text-black  w-full'>
                  ...
                  <FormControl className='border  rounded-3xl border-gray shadow-none p-0 text-black placeholder:text-black focus-visible:outline-none'>
                    <Input
                      className='h-12 w-full px-6 rounded-3xl'
                      placeholder={`${t('name')}*`}
                      {...field}
                    />
                  </FormControl>
                          ...
                          <Checkbox
                            ...
                            onCheckedChange={checked => {
                              return checked
                                ...
                                : field.onChange(field.value?.filter(value => value !== 'TEL'));
                            }}
                            id='TEL'
                            className='border-gray size-4 cursor-pointer'
                          />
            ...
            render={({ field }) => (
              <FormItem className='col-span-full mt-2 lg:mt-4'>
                <div className='flex gap-2 col-span-full mt-2 lg:mt-4'>
                  ...
                  <div
                    className={`${form.formState.errors.termsAccepted ? 'text-destructive animate-shake' : ''}`}
                  >
                    {t('agreement_slice_1')}{' '}
          ...
          <Button
            type='submit'
            className=' hover:bg-blue-2 text-white  text-base cursor-pointer col-span-full h-12 rounded-3xl bg-blue-2 mt-8 mb-24 font-semibold'
          >
            {t('send')}
          </Button>
        </form>
