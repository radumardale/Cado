...
export default function AdminHeader({page, href, id}: AdminHeaderProps) {
  ...
  return (
    <div className='col-span-12 pt-8 flex justify-between items-center h-fit'>
        <div className="flex">
            <Link href={href} className='font-manrope text-2xl font-semibold leading-7'>{t(`admin_pages.${page}.title`)}</Link>
            {
                id && <p className='font-manrope text-2xl font-semibold leading-7'>&nbsp;/ {t(`admin_pages.${page}.item`)} {id}</p>
            }
        </div>
