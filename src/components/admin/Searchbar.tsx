...
export default function Searchbar({searchText, setSearchText, className}: SearchbarProps) {
  ...
  return (
    <div className={`relative h-12 ${className}`}>
        ...
        <Input type='text' className='h-12 bg-background border-none rounded-3xl pl-12 text-black' placeholder='Caută...'  value={searchText} onChange={(e) => {setSearchText(e.target.value)}} />
