...
export default function ProductTagsSearchbar({
  handleAddCategory,
  handleAddOcasion,
  handleAddProductContent,
}: ProductTagsSearchbarProps) {
  ...
  return (
    <>
      ...
      <p
        className={`${categories.length > 0 ? '' : 'hidden'} col-span-full font-manrope font-semibold`}
      >
        {prod_t('main_category')}:
      </p>
      <div className={`${categories.length > 0 ? 'flex' : 'hidden'} col-span-6 gap-2 flex-wrap`}>
        {categories.map((category, index) => {
          return (
            <button
              key={index}
              className='bg-blue-2 h-12 px-6 text-white font-manrope font-semibold rounded-3xl flex items-center gap-1 max-w-full cursor-pointer hover:opacity-75 transition duration-300'
              onClick={e => {
                e.preventDefault();
      ...
      <p
        className={`${productContents.length > 0 ? '' : 'hidden'} col-span-full font-manrope font-semibold`}
      >
        {prod_t('content')}:
      </p>
      <div
        className={`${productContents.length > 0 ? 'flex' : 'hidden'} col-span-6 gap-2 flex-wrap`}
      >
        {productContents.map((productContent, index) => {
          return (
            <button
              ...
              onClick={e => {
                e.preventDefault();
                handleAddProductContent(productContent);
              }}
            >
              <p className='whitespace-nowrap overflow-hidden text-ellipsis'>
                {t(`product_content.${productContent}`)}
              </p>
            </button>
          );
        })}
      ...
      <p
        className={`${ocasions.length > 0 ? '' : 'hidden'} col-span-full font-manrope font-semibold`}
      >
        {prod_t('ocasion')}:
      </p>
        ...
        {ocasions.map((ocasion, index) => {
          return (
            <button
              ...
              onClick={e => {
                e.preventDefault();
                handleAddOcasion(ocasion);
              }}
            >
              <p className='whitespace-nowrap overflow-hidden text-ellipsis'>
                {t(`ocasions.${ocasion}.title`)}
              </p>
            </button>
          );
        })}
