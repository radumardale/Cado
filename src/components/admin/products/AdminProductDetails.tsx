  ...
  return (
    <>
      ...
      <div className='xl:col-start-2 col-span-7 xl:col-span-6 grid grid-cols-6 gap-x-6 gap-y-4 pb-8'>
        ...
        />
        <ProductTagsSearchbar
          handleAddCategory={handleAddCategory}
          handleAddOcasion={handleAddOcasion}
          handleAddProductContent={handleAddProductContent}
        />
              ...
              {categories.map((category: Categories, index: number) => {
                return (
                  <p key={index} className='text-gray'>
                    ...
                    <span
                      className={`${index === categories.length - 1 ? 'hidden' : ''} text-black`}
                    >
                      ,
                    </span>{' '}
                  </p>
              ...
              {productContents.map((productContent: ProductContent, index: number) => {
                return (
                  <p key={index} className='text-gray'>
                    ...
                    <span
                      className={`${index === productContents.length - 1 ? 'hidden' : ''} text-black`}
                    >
                      ,
                    </span>{' '}
                  </p>
              ...
              {ocasions.map((ocasion: Ocasions, index: number) => {
                return (
                  <p key={index} className='text-gray'>
                    ...
                    <span className={`${index === ocasions.length - 1 ? 'hidden' : ''} text-black`}>
                      ,
                    </span>{' '}
                  </p>
                );
            ...
            {productContents.map((productContent: ProductContent, index: number) => {
              return (
                <ActiveFiltersButton
                  ...
                  onClick={() => {
                    const updatedProductContents = productContents.filter(
                      (content: ProductContent) => content !== productContent
                    );
                    form.setValue('data.product_content', updatedProductContents, {
                      shouldDirty: true,
                    });
                  }}
                />
