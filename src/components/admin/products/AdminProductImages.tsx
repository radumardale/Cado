      ...
      <>
              ...
              <div className='fixed top-0 left-0 w-full h-full bg-black/75 z-50 flex justify-center items-center' onMouseDown={() => {setIsDeleteDialogOpen(false)}}>
                  <div className='p-8 rounded-3xl bg-white' onMouseDown={(e) => {e.stopPropagation()}}>
                      ...
                      <div className='flex gap-6 ml-36 mt-12'>
                          ...
                          <button onClick={(e) => {e.preventDefault(); mutate({id: product?._id})}} className='disabled:opacity-75 disabled:cursor-default cursor-pointer h-12 px-6 flex justify-center items-center bg-red text-white rounded-3xl hover:opacity-75 transition duration-300'>{t("delete")}</button>
                      </div>
                  </div>
              </div>
          }
          <div data-lenis-prevent className='col-span-4 col-start-9 flex-1 overflow-y-auto mt-16 pb-8 grid grid-cols-4 gap-x-6'>
              <div className='col-span-full grid grid-cols-4 gap-x-6 h-fit'>
                  <div className='col-span-full grid grid-cols-4 gap-x-6 h-fit'>
                      <p className='font-manrope text-2xl font-semibold leading-7 mb-6 col-span-full'>{t('images')}</p>
                  ...
                  <div className='col-span-full mt-8 h-fit grid grid-cols-4 gap-x-6 gap-y-4'>
                      <FormField
                          ...
                          render={({ field }) => (
                              <FormItem className='col-span-3'> 
                                  <FormMessage />
                                      <FormLabel className="font-semibold font-manrope leading-5">{t('availability')}</FormLabel>
                                      <Select onValueChange={field.onChange} value={isMounted ? field.value : StockState.IN_STOCK}>
                                          ...
                                          <SelectContent className="border-gray">
                                              <SelectGroup>
                                                  <SelectItem className="text-base cursor-pointer" value={StockState.IN_STOCK}>{t('in_stock')}</SelectItem>
                                                  <SelectItem className="text-base cursor-pointer" value={StockState.ON_COMMAND}>{t('on_command')}</SelectItem>
                                                  <SelectItem className="text-base cursor-pointer" value={StockState.NOT_IN_STOCK}>{t('not_in_stock')}</SelectItem>
                                              </SelectGroup>
                                          </SelectContent>
                                      </Select>
                              </FormItem>
                      ...
                      <FormField
                          ...
                          render={({ field }) => (
                              <FormItem className="text-black col-span-1">
                                  <FormLabel className="font-semibold font-manrope leading-5 whitespace-nowrap">{t('stock_nr')}</FormLabel>
                                  <FormMessage />
                      ...
                      <div className="col-span-3 flex gap-6">
                          <FormField
                              ...
                              render={({ field }) => (
                                  <FormItem className="text-black col-span-1">
                                      <FormLabel className="font-semibold font-manrope leading-5 whitespace-nowrap">{isSaleActive ? t("old_price") : t("price")}</FormLabel>
                                      <FormMessage />
                          ...
                          <FormField
                              ...
                              render={({ field }) => (
                                  <FormItem className={`${isSaleActive ? "" : "invisible pointer-events-none"} text-black col-span-1`}>
                                      <FormLabel className="font-semibold font-manrope leading-5 whitespace-nowrap">{t("new_price")}</FormLabel>
                                      <FormMessage />
                                      <div className='flex gap-2 items-center'>
                                          <FormControl className="border rounded-3xl border-gray shadow-none p-0 text-black placeholder:text-black focus-visible:outline-none">
                                              <Input 
                      ...
                      <FormField
                          ...
                          render={({ field }) => (
                              <>
                                  <FormItem className="text-black col-span-1">
                                      <FormLabel className="font-semibold font-manrope leading-5 whitespace-nowrap">{t('discount')}</FormLabel>
                                      <FormMessage />
                                      <FormControl className="border rounded-3xl border-gray shadow-none p-0 text-black placeholder:text-black focus-visible:outline-none">
                                      <Switch
              ...
              <div className='flex justify-between gap-6 flex-1 items-end mt-6 col-span-full'>
                  <button onClick={(e) => {e.preventDefault(); setIsDeleteDialogOpen(true)}} className='disabled:opacity-75 disabled:cursor-default cursor-pointer h-12 px-6 flex justify-center items-center bg-red text-white rounded-3xl hover:opacity-75 transition duration-300'>{t('delete_product')}</button>
                  ...
                  <div className='flex gap-6'>
                      ...
                      <button disabled={!isDirty} className='disabled:opacity-75 disabled:cursor-default cursor-pointer h-12 px-6 flex justify-center items-center bg-blue-2 text-white rounded-3xl hover:opacity-75 transition duration-300'>{t("save")}</button>
                  </div>
              </div>
          </div>
      </>
