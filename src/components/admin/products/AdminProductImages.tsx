...
export default function AdminProductImages({
  ...
  useEffect(() => {
    if (
      form.getValues('data.imagesNumber') !==
      imagesData.filter(image => !image.startsWith('https')).length
    ) {
      form.setValue(
        'data.imagesNumber',
        imagesData.filter(image => !image.startsWith('https')).length,
  ...
  return (
    <>
      ...
      <div
        data-lenis-prevent
        className='col-span-4 col-start-9 flex-1 overflow-y-auto mt-16 pb-8 grid grid-cols-4 gap-x-6'
      >
        <div className='col-span-full grid grid-cols-4 gap-x-6 h-fit'>
          <div className='col-span-full grid grid-cols-4 gap-x-6 h-fit'>
            <p className='font-manrope text-2xl font-semibold leading-7 mb-6 col-span-full'>
              {t('images')}
            </p>
            <div className='col-span-4 grid grid-cols-4 gap-4'>
              <ProductImageUpload onImageAdded={handleImageAdded} />
              {imagesData.map((image, index) => {
                return (
                  <div key={index} className='col-span-1 relative group'>
                    ...
                    <Image
                      src={image}
                      alt={product ? product.title[locale] : 'new image'}
                      width={339}
                      height={339}
              ...
              render={({ field }) => (
                <FormItem className='col-span-3'>
                  ...
                  <Select
                    ...
                    value={isMounted ? field.value : StockState.IN_STOCK}
                  >
                    <FormControl>
                    ...
                    <SelectContent className='border-gray'>
                      <SelectGroup>
                        ...
                        <SelectItem
                          className='text-base cursor-pointer'
                          value={StockState.ON_COMMAND}
                        >
                          {t('on_command')}
                        </SelectItem>
                        <SelectItem
                          className='text-base cursor-pointer'
                          value={StockState.NOT_IN_STOCK}
                        >
                          {t('not_in_stock')}
                        </SelectItem>
                      </SelectGroup>
            ...
            <div className='col-span-3 flex gap-6'>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='text-black col-span-1'>
                    <FormLabel className='font-semibold font-manrope leading-5 whitespace-nowrap'>
                      {isSaleActive ? t('old_price') : t('price')}
                    </FormLabel>
                    <FormMessage />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem
                    className={`${isSaleActive ? '' : 'invisible pointer-events-none'} text-black col-span-1`}
                  >
                    <FormLabel className='font-semibold font-manrope leading-5 whitespace-nowrap'>
                      {t('new_price')}
                    </FormLabel>
          ...
          <div className='flex gap-6'>
            <button
              ...
              onClick={e => {
                ...
                form.setValue(
                  'data.imagesNumber',
                  initialImagesData.filter(img => !img.startsWith('https')).length
                );
