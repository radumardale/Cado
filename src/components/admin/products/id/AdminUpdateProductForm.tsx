...
export default function AdminUpdateProductForm({ id }: AdminProductFormProps) {
  ...
  const {
    ...
    mutate,
    data: MutatedData,
  } = useMutation(trpc.products.updateProduct.mutationOptions());
  const {
    mutate: UpdateMutate,
    isSuccess: UpdateIsSuccess,
    data: UpdateData,
  } = useMutation(trpc.image.uploadProductImages.mutationOptions());
    ...
    <>
      ...
      <Form {...form}>
        ...
        >
          <div
            data-lenis-prevent
            className='col-span-7 grid grid-cols-7 scroll-bar-custom overflow-y-auto flex-1 -mr-6 pr-6 mt-16'
          >
            <AdminProductDetails />
          </div>
          <AdminProductImages
