                    ...
                    <Link href={{pathname: "/admin/products/[id]", params: {id: product.custom_id}}} key={index} className='grid grid-cols-12 col-span-12 gap-x-6 h-18 border border-gray hover:border-black transition duration-300 rounded-3xl items-center mb-4 last-of-type:mb-6'>
                        ...
                        <div className='col-span-2 translate-x-1/2 w-[calc(50%-.75rem)]'>
                            <div className='bg-purewhite w-fit rounded-sm'>
                                <Image src={product.images[0]} alt={product.title[locale]} width={96} height={128} className='w-12 h-16 rounded-sm object-contain' />
                            </div>
                        </div>
                        ...
                        <div className='col-span-2 font-semibold flex gap-0'>
                            <p className='flex-1'>{product.sale && product.sale.active ? product.sale.sale_price.toLocaleString() : product.price.toLocaleString()} MDL</p>
                            {
                                product.sale && product.sale.active && <p className='text-gray line-through flex-1'>{product.price.toLocaleString()} MDL</p>
                            }
                        </div>
                        <p className='col-span-2 font-semibold' style={{color: stockStateColors[product.stock_availability.state]}}>{t(product.stock_availability.state)}</p>
