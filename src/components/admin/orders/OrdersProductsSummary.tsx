...
export default function OrdersProductsSummary() {
  ...
  const formValues = useWatch({
    ...
    name: [
      'additional_info.delivery_address.region',
      'delivery_details.hours_intervals',
      'products',
    ],
  });
  ...
  useEffect(() => {
    ...
    const subtotal = items.reduce(
      (acc, item) =>
        acc +
        (item.product.sale?.active ? item.product.sale.sale_price : item.product.price) *
          (item.quantity || 1),
      0
    );

    const deliveryPriceTotal = deliveryPrice
      ? deliveryPrice + (deliveryHourRate ? deliveryHourRate * deliveryPrice : 0)
      : 0;
    ...
    <>
      ...
      <div className='col-span-full lg:col-start-9 lg:col-span-4 left-0 flex-1 overflow-auto pb-16 flex flex-col justify-between mt-16 relative'>
        ...
        {items.length > 0 && mounted ? (
          <div
            data-lenis-prevent
            className='flex flex-col pr-2 gap-6 flex-1 lg:overflow-y-auto scroll-bar-custom mb-16 lg:mb-8'
          >
                ...
                <div key={index} className='w-full flex gap-2 lg:gap-4'>
                  <Link
                    href={{
                      ...
                      params: { id: item.product.custom_id || item.product._id },
                    }}
                    className='peer'
                  >
                    <Image
                      src={item.product.images?.[0] || '/placeholder.jpg'}
                      alt={item.product.title?.[locale] || 'Product'}
                      width={129}
                      height={164}
                      className='w-32 aspect-[129/164] object-cover rounded-lg peer'
                    />
                  </Link>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            newItems[index].quantity = Math.min(
                              newItems[index].product.stock_availability.stock,
