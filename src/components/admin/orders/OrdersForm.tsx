...
import {
  ...
  UpdateOrderValues,
} from '@/lib/validation/order/updateOrderRequest';
import { z } from 'zod';
...
export default function OrdersForm({ orderId }: { orderId: string }) {
  ...
  const {
    ...
    mutate,
    data: MutatedData,
  } = useMutation(trpc.order.updateOrder.mutationOptions());
      ...
      <div className='col-span-7 xl:col-span-6 col-start-1 xl:col-start-2 grid grid-cols-6 gap-x-2 lg:gap-x-6 flex-1 pb-16'>
        ...
        >
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='col-span-full lg:col-span-3 mb-8'>
                <FormMessage />
                <Select onValueChange={field.onChange} value={field.value || 'NOT_PAID'}>
                  <FormControl>
                    <SelectTrigger
                      className='cursor-pointer flex h-12 max-h-none items-center px-6 gap-2 border border-gray rounded-3xl text-base text-black w-full'
                      style={{
                        color: orderStateColors[field.value as keyof typeof orderStateColors],
                      }}
                    >
                  ...
                  <SelectContent className='border-gray'>
                    <SelectGroup>
                      <SelectItem
                        ...
                        style={{ color: orderStateColors['TRANSACTION_FAILED'] }}
                      ...
                      <SelectItem
                        className='text-base cursor-pointer'
                        style={{ color: orderStateColors['NOT_PAID'] }}
                        value='NOT_PAID'
                      >
                      ...
                      <SelectItem
                        className='text-base cursor-pointer'
                        style={{ color: orderStateColors['PAID'] }}
                        value='PAID'
                      >
                      ...
                      <SelectItem
                        className='text-base cursor-pointer'
                        style={{ color: orderStateColors['DELIVERED'] }}
                        value='DELIVERED'
                      >
          ...
          {deliveryMethod === DeliveryMethod.PICKUP && (
            <>
              <p className=' text-2xl font-semibold leading-7 mb-2 lg:mb-6 col-span-full mt-8 lg:mt-12'>
                {checkout_t('pickup_place')}
              </p>
              <p className='col-span-full'>
                {checkout_t('pickup_info_slice_1')}{' '}
                <span className='font-semibold'>{checkout_t('pickup_info_slice_2')} </span>{' '}
              </p>
            </>
          )}

          <p className=' text-2xl font-semibold leading-7 mb-2 col-span-full mt-8 lg:mt-12 font-manrope'>
            {checkout_t('customer_info')}
          </p>
            ...
            render={({ field }) => (
              <FormItem className='col-span-full lg:col-span-3 mt-2 lg:mt-4'>
                ...
                <FormControl className='border  rounded-3xl border-gray shadow-none p-0 text-black placeholder:text-black focus-visible:outline-none'>
                  <Input
                    className='h-12 w-full px-6 rounded-3xl text-base text-black'
                    placeholder={`${checkout_t('first_name')}*`}
                    {...field}
                  />
                </FormControl>
              </FormItem>
            ...
            render={({ field }) => (
              <FormItem className='col-span-full lg:col-span-3 mt-2 lg:mt-4'>
                ...
                <FormControl className='border  rounded-3xl border-gray shadow-none p-0 text-black placeholder:text-black focus-visible:outline-none'>
                  <Input
                    className='h-12 w-full px-6 rounded-3xl text-base text-black'
                    placeholder={`${checkout_t('last_name')}*`}
                    {...field}
                  />
                </FormControl>
              </FormItem>
                ...
                render={({ field }) => (
                  <FormItem className='col-span-full lg:col-span-3 mt-2 lg:mt-4'>
                    ...
                    <FormControl className='border  rounded-3xl border-gray shadow-none p-0 text-black placeholder:text-black focus-visible:outline-none'>
                      <Input
                        className='h-12 w-full px-6 rounded-3xl text-base text-black'
                        placeholder={checkout_t('address')}
                        {...field}
                      />
                    </FormControl>
                  </FormItem>
                        ...
                        >
                          {DeliveryRegionsArr.map((region, index) => {
                            return (
                              ...
                              >
                                {t(`CheckoutPage.CheckoutForm.delivery_regions.${region}`)}
                              </SelectItem>
                            );
                          })}
                        </SelectGroup>
          ...
          {deliveryMethod === DeliveryMethod.HOME_DELIVERY && (
            <div className='flex mb-6 col-span-full gap-2'>
              ...
              />
              <label htmlFor='billing_address_check'>
                {checkout_t('billing_address_checkbox')}
              </label>
            </div>
          )}
                    ...
                    render={({ field }) => (
                      <FormItem className='col-span-full lg:col-span-3 mt-2 lg:mt-4'>
                        ...
                        <FormControl className='border  rounded-3xl border-gray shadow-none p-0 text-black placeholder:text-black focus-visible:outline-none'>
                          <Input
                            className='h-12 w-full px-6 rounded-3xl text-base text-black'
                            placeholder={`${checkout_t('company_name')}*`}
                            {...field}
                          />
                        </FormControl>
                      </FormItem>
                    ...
                    render={({ field }) => (
                      <FormItem className='col-span-full lg:col-span-3 mt-2 lg:mt-4'>
                        ...
                        <FormControl className='border  rounded-3xl border-gray shadow-none p-0 text-black placeholder:text-black focus-visible:outline-none'>
                          <Input
                            className='h-12 w-full px-6 rounded-3xl text-base text-black'
                            placeholder={`${checkout_t('company_id')}*`}
                            {...field}
                          />
                        </FormControl>
                      </FormItem>
                        ...
                        >
                          {DeliveryRegionsArr.map((region, index) => {
                            return (
                              ...
                              >
                                {
                                  t(`CheckoutPage.CheckoutForm.delivery_regions.${region}`).split(
                                    ' - '
                                  )[0]
                                }
                              </SelectItem>
                        ...
                        <SelectGroup>
                          {DeliveryHoursArr.map((hour, index) => {
                            return (
                              ...
                              >
                                {t(`CheckoutPage.CheckoutForm.delivery_hours.${hour}`)}
                              </SelectItem>
                            );
                          })}
                        </SelectGroup>
            ...
            render={({ field }) => (
              <FormItem className='col-span-full lg:col-span-3 mt-2 lg:mt-4'>
                <FormControl>
                  <Textarea
                    className='placeholder:text-black h-40 items-center px-6 border border-gray rounded-3xl text-base text-black col-span-full'
                    placeholder={checkout_t('comment')}
                    {...field}
                  />
                </FormControl>
                <FormMessage />
