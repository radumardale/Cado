...
export default function AdminRecommendationsCard({
  product,
  setAddProductOpen,
  ...
  return (
    <div
      className={`flex flex-col h-full col-span-4 lg:col-span-3 group cursor-pointer`}
      ...
      <div className='relative overflow-hidden before:content-[""] before:absolute before:left-0 before:top-0 before:w-full before:h-full before:bg-pureblack before:opacity-0 group-hover:before:opacity-25 before:rounded-lg before:lg:rounded-2xl before:transition before:duration-300 before:z-10 mb-4 aspect-[339/425]'>
        ...
        <div className='bg-purewhite w-full h-full rounded-lg lg:rounded-2xl overflow-hidden opacity-100 group-hover:opacity-0 z-10 transition duration-300 relative'>
          <Image
            ...
            className='max-w-full w-fit max-h-full object-contain z-10 absolute left-1/2 top-1/2 -translate-1/2'
          />
        </div>
        <div className='bg-purewhite w-full h-full absolute left-0 top-0 transition duration-300 -z-10 rounded-lg lg:rounded-2xl overflow-hidden'>
          <Image
            ...
            height={2396}
            alt={product.title.ro}
            className={`${isImageLoaded ? '' : 'hidden'} absolute left-0 top-1/2 -translate-y-1/2 max-w-full max-h-full object-contain`}
          />
        </div>
        <button className='absolute left-4 -bottom-12 h-12 w-[calc(100%-2rem)] bg-white rounded-3xl font-manrope z-20 opacity-100 transition-all duration-300 group-hover:bottom-4 font-semibold cursor-pointer hover:bg-lightergray'>
          {t('change_prod')}
        </button>
      </div>
      <div className='col-span-3 group cursor-pointer flex flex-col flex-1'>
        <p className={`flex-1 font-manrope font-semibold mb-2 text-left`}>
        ...
        <div className={`flex gap-1 items-center lg:mx-0`}>
          ...
          <div
            className={`font-manrope font-semibold border border-gray rounded-3xl w-fit mx-0 px-2 lg:px-4 py-2`}
          >
            {product.sale && product.sale.active
