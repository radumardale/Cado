...
export default function HomeBannerContent() {
  const trpc = useTRPC();
  const { data: HomeBannerQuery, refetch } = useQuery(
  ...
  const currentBanner =
    slideNumber < totalExistingBanners ? HomeBannerQuery?.banners[slideNumber] : null;
  ...
  return (
    <>
      <p className='mt-[3rem] font-manrope text-[1.5rem] font-bold mb-[1.5rem] col-span-full'>
        {t('banner_ro')}
      </p>
      <div className='col-span-full overflow-hidden rounded-2xl lg:rounded-3xl relative mb-6'>
        <ArrowRight
          className='absolute top-1/2 -translate-y-1/2 right-2 lg:right-6 z-20 text-blue-4 cursor-pointer'
          onClick={nextSlide}
        />
        <ArrowLeft
        ...
        <div className='flex gap-4 lg:gap-6 absolute w-full lg:w-fit left-1/2 -translate-x-1/2 bottom-2 lg:bottom-6 z-20 px-4 lg:px-0'>
          {Array.from({ length: totalSlides }).map((_, index) => {
            return (
              <div
                key={index}
                className='lg:py-4 cursor-pointer flex-1 lg:flex-auto'
                onClick={() => {
                  setToSlide(index);
                }}
              >
                <button
                  className={`h-0.5 w-full lg:w-18 pointer-events-none transition duration-200 rounded-2xl ${
