...
export default function AdminSeasonCatalog() {
    ...
    const { mutate, isSuccess, data: MutatedData } = useMutation(trpc.seasonCatalog.updateSeasonCatalog.mutationOptions());
    ...
    useEffect(() => {
        if (isSuccess) {
            toast.success("Catalog sezon modificat cu succes!")
            form.reset({
    ...
    return (
      <>
          <h2 className='col-span-5 font-manrope font-semibold text-3xl leading-11 mb-10 mt-42'>CATALOG DE SEZON</h2>
          <h3 className='col-start-1 col-span-4 font-manrope font-semibold leading-7'>Detalii catalog de sezon</h3>
          <Form {...form}>
                          ...
                          render={({ field }) => (
                              <>
                                  <FormItem className="text-black col-span-3">
                                      <FormLabel className="font-semibold font-manrope leading-5 whitespace-nowrap">Catalog de sezon</FormLabel>
                                      <FormMessage />
                      ...
                      <FormField
                          ...
                          render={({ field }) => (
                              <FormItem className="text-black col-span-6">
                                  <FormLabel className="font-semibold font-manrope leading-5">Link spre catalog</FormLabel>
                                  <FormMessage className='top-7' />
                                  <FormControl className="border rounded-3xl border-gray shadow-none p-0 text-black placeholder:text-black focus-visible:outline-none">
                                      <Input className=" text-base h-12 w-full px-6 rounded-3xl" placeholder="Link-ul*" {...field} />
                                  </FormControl>
                              </FormItem>
                          )}
                      />
