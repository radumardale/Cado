...
export default function Breadcrums({ category, productInfo }: BreadcrumsInterface) {
  ...
  const pathNames = paths
    .split('/')
    .filter(path => !LangOptionsArr.includes(path) && path.length > 0)
    ...
    <div className='mt-4 hidden lg:flex'>
      {
        <div className='flex gap-2'>
          <Link
            href={'/'}
            className='text-gray font-semibold font-manrope uppercase whitespace-nowrap'
          >
            {path_t('home')}
          </Link>
          ...
          {pathNames.map((link, index) => {
            ...
            const itemLink =
              link === 'katalog' ? 'Каталог' : link[0].toUpperCase() + link.slice(1, link.length);
            ...
            if (productInfo && index == 1) {
              return (
                <Fragment key={index}>
                  <Link
                    className='text-gray font-semibold font-manrope uppercase whitespace-nowrap'
                    href={{ pathname: '/catalog/product/[id]', params: { id: productInfo.id } }}
                  >
                    {productInfo.title}
                  </Link>
                </Fragment>
