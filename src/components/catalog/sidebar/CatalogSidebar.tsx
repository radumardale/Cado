...
export default function CatalogSidebar({
  ...
  const updateOcasions = useCallback(
    (value: Ocasions) => {
  ...
  const updateProductContent = useCallback(
  ...
  return (
    <motion.div
      ...
      animate={
        isSidebarOpen
          ? { x: 0, transition: { duration: 0.4, ease: easeInOutCubic } }
          : { x: '-100%', transition: { duration: 0.4, ease: easeInOutCubic } }
      }
      data-lenis-prevent
    ...
    >
      <button
        className='cursor-pointer absolute right-4 top-4 lg:hidden'
        onClick={() => {
      ...
      <LayoutGroup>
        <AnimatePresence>
          {(categoriesState.category ||
            ocasionsState.ocasions.length > 0 ||
            productContentState.productContent.length > 0 ||
            (priceState.price[0] !== minPrice && priceState.price[0] !== 0) ||
            (priceState.price[1] !== maxPrice && priceState.price[1] !== 0) ||
            keywordsState.keywords !== null) && (
            <ActiveFilters
              ...
              updateOcasions={(
                ...
                ocasionsState.setOcasions(ocasionsState.ocasions.filter(item => item !== value));
                checkboxUpdateUrlParams(
                  'ocasions',
                  searchParams,
                  router,
        ...
        <Accordion title={t('category')} last>
          <CategoriesGrid
            ...
            setCategory={(value: Categories | null) =>
              categoriesState.setCategory(value === categoriesState.category ? null : value)
            }
          />
        </Accordion>
      ...
      <button
        ...
        className='h-12 w-full bg-blue-2 text-white rounded-3xl font-manrope font-semibold cursor-pointer border mt-10 mb-16'
      >
        {t('update')}
      </button>
    </motion.div>
