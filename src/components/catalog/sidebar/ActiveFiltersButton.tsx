...
export const ActiveFiltersButton = ({ title, onClick }: ActiveFiltersButtonInterface) => {
  return (
    <button
      className='bg-black h-12 px-6 text-white font-manrope font-semibold rounded-3xl flex items-center gap-1 max-w-full cursor-pointer hover:opacity-75 transition duration-300'
      onClick={e => {
        e.preventDefault();
        onClick();
      }}
