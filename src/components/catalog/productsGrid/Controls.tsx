...
export default function Controls({
  ...
  return (
    <div className='col-span-full flex gap-1 lg:gap-2 justify-end sticky lg:relative top-14 lg:top-auto py-2 lg:py-0 left-0 z-40 bg-white lg:z-auto lg:bg-transparent items-center'>
      {searchText && (
        <h2 className='font-manrope text-2xl leading-7 uppercase font-semibold mr-auto'>
          {countProducts} {countProducts === 1 ? 'REZULTAT' : 'REZULTATE'} CU “
      ...
      <div className='flex gap-1 lg:gap-2 w-full lg:w-auto'>
        <button
          ...
          className={`lg:hidden mr-auto size-12 rounded-full border border-black flex justify-center items-center transition duration-300 hover:bg-black cursor-pointer ${isSidebarOpen ? 'bg-black text-white' : ''} hover:text-white`}
        >
