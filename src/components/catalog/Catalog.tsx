...
export default function Catalog() {
  ...
  const [category, setCategory] = useState<Categories | null>(
    (searchParams.get('category') as Categories) || null
  );
  const [ocasions, setOcasions] = useState<Ocasions[]>(
    searchParams.getAll('ocasions') as Ocasions[]
  );
  const [productContent, setProductContent] = useState<ProductContent[]>(
    searchParams.getAll('product_content') as ProductContent[]
  );
  const [price, setPrice] = useState<number[]>([0, 0]);
  const [sortBy, setSortBy] = useState<SortBy>(
    searchParams.get('sort_by') ? (searchParams.get('sort_by') as SortBy) : SortBy.RECOMMENDED
  );
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } = useInfiniteQuery(
    trpc.products.getProducts.infiniteQueryOptions(queryData, {
