...
export default function CatalogMenu({ setIsCatalogMenuActive }: CatalogMenuProps) {
  ...
  return (
    <div
      className={`hidden lg:grid col-start-1 col-span-full lg:grid-cols-13 2xl:grid-cols-full gap-x-6 pt-4 ${searchText.length < 2 ? 'pb-6' : 'pb-0'} relative z-[49]`}
      onMouseEnter={() => {
        setIsCatalogMenuActive(true);
      }}
      onMouseLeave={() => {
        setIsCatalogMenuActive(false);
      }}
    >
      <Image
        src='/ribbon/left-bottom-ribbon.png'
        className='absolute left-0 bottom-0 w-[13vw]'
        width={242}
        height={225}
        alt='ribbon'
      />
      ...
      <Image
        src='/ribbon/left-bottom-ribbon.png'
        className='absolute right-0 bottom-0 w-[13vw] rotate-y-180'
        width={242}
        height={225}
        alt='ribbon'
      />
