...
export default function SearchProducts({products, productsCount, searchText, closeMenu}: SearchProductsProps) {
    const locale = useLocale();
    const router = useRouter();
    const [value, setValue] = useLocalStorage<CartInterface[]>('cart', []);
    const [isImageLoaded, setImageLoaded] = useState(false);
    const [localProducts, setLocalProducts] = useState<ProductInterface[]>(products ? products : []);
    const [localCount, setLocalCount] = useState<number>(productsCount === undefined || productsCount === null ? 0 : productsCount)
  ...
  return (
    <div className='col-span-full h-full lg:h-auto lg:mx-auto'>
        ...
        <div className='col-span-full h-full lg:h-80 relative'>
            {
                localCount < 1 &&
                        ...
                        return (
                            <div key={index} className='w-44 flex flex-col group' onClick={() => {router.push({pathname: "/catalog/product/[id]", params: {id: product.custom_id}})}}>
                                ...
                                <Link href={{pathname: "/catalog/product/[id]", params: {id: product.custom_id}}} className='group cursor-pointer flex flex-col flex-1 justify-between'>
                                    ...
                                    <div className={`font-manrope font-semibold border border-gray rounded-3xl w-fit mx-0 px-2 py-1.5`}>{product.price.toLocaleString()} MDL</div>
                                </Link>
                            </div>
                        )
                    })
