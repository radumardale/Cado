...
export default function SearchProducts({
  ...
  return (
    <div className='col-span-full h-full lg:h-97.5 lg:mx-auto mb-[1rem]'>
      {localCount > 0 && (
        <button
          className='font-manrope font-semibold border border-black rounded-3xl w-fit px-6 h-12 flex items-center gap-2 mx-auto cursor-pointer mb-4'
          onClick={() => {
            router.push({
              pathname: '/catalog',
              query:
                productsCount !== undefined && productsCount !== null && productsCount > 0
                  ? {
                      keywords: searchText
                        .split(/\s+/)
      ...
      <div className='col-span-full relative h-full'>
        {localCount == 0 && (
          <div className='absolute left-1/2 top-1/2 -translate-1/2 w-full lg:w-auto px-13 lg:px-0 flex flex-col items-center'>
            <Image
              src='/icons/shopping-bag-sad.svg'
              alt='sad shopping bag'
              width={48}
              height={48}
              className='size-12'
            />
            <p className='mt-2 text-sm leading-4 lg:leading-5 lg:text-base text-center mb-12'>
              {t('no_match')}
            </p>
            <Link
              href={'/catalog'}
              className='font-manrope font-semibold bg-blue-2 rounded-3xl w-fit px-6 py-3.5 flex gap-2 mx-auto cursor-pointer hover:opacity-75 transition duration-300'
              onClick={() => {
                router.push({
                  pathname: '/catalog',
                  query:
            ...
            >
              <span className='font-semibold leading-5 font-manrope text-white'>
                {t('see_catalog')}
              </span>
            </Link>
          </div>
          ...
          {localCount > 0 &&
            localProducts.map((product, index) => {
              return (
                <div
                  key={index}
                  className='w-44 flex flex-col group'
                  onClick={() => {
                    router.push({
                      pathname: '/catalog/product/[id]',
                      params: { id: product.custom_id },
                    });
                  }}
                >
                  <div className='relative overflow-hidden before:content-[""] before:absolute before:left-0 before:top-0 before:w-full before:h-full before:bg-pureblack before:opacity-0 group-hover:before:opacity-25 before:rounded-lg before:lg:rounded-2xl before:transition before:duration-300 before:z-10 mb-2'>
                    ...
                    <div className='absolute left-0 top-0 h-full w-full rounded-lg lg:rounded-2xl transition duration-300 -z-10 bg-purewhite flex items-center'>
                      <Image
                        ...
                        height={1198}
                        alt={product.title.ro}
                        className={`${isImageLoaded ? '' : 'hidden'} w-full max-h-full object-contain`}
                      />
                    </div>
                    <button
                      onClick={e => {
                        e.stopPropagation();
                        addToCart(product, 1, value, setValue, locale);
                      }}
                      className='absolute left-2 -bottom-10 h-10 w-[calc(100%-1rem)] bg-white rounded-3xl font-manrope z-20 opacity-100 transition-all duration-300 group-hover:bottom-2 font-semibold cursor-pointer hover:bg-lightgray text-sm'
                    >
                      {t('add_to_cart')}
                    </button>
                  </div>
                  <Link
                    href={{ pathname: '/catalog/product/[id]', params: { id: product.custom_id } }}
                    className='group cursor-pointer flex flex-col flex-1 justify-between'
                  >
                    <p className={`font-manrope font-semibold mb-2 text-left text-sm leading-4`}>
                      {product.title[locale]}
                    </p>
                    <div className={`flex gap-1 items-center`}>
                      {product.sale && product.sale.active && (
                        <p className='text-gray text-sm leading-4 font-semibold line-through'>
                          {product.price.toLocaleString()} MDL
                        </p>
                      )}
                      <div
                        className={`font-manrope font-semibold border border-gray rounded-3xl w-fit px-2 py-1.5`}
                      >
                        {product.sale && product.sale.active
