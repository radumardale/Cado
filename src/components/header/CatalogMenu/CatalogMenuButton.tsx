...
export default function CatalogMenuButton({isCatalogMenuOpen, setCatalogButtonActive}: CatalogMenuButtonProps) {
  return (
    <div className={`flex items-center gap-2 rounded-3xl h-12 py-3 px-6 border border-black transition duration-300 relative cursor-pointer ${isCatalogMenuOpen ? "text-white" : ""}`} onMouseEnter={() => {setCatalogButtonActive(true)}} onMouseLeave={() => {setCatalogButtonActive(false)}}>
        <motion.div 
