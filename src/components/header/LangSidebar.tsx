...
import {
  CountriesOptions,
  CountriesOptionsArr,
  CountriesOptionsInterface,
} from '@/lib/enums/CountriesOptions';
      ...
      >
        ...
        <div className='xl:px-12 z-100'>
          ...
          >
            ...
            <SelectContent
              className='border-gray z-[9999] top-[3rem] left-0 px-4 rounded-2xl'
              position='popper'
            >
          ...
          <Link
            shallow
            href={`${cleanedPathname}${usableSearchParams}` as Pathnames}
            scroll={false}
            locale={language}
          >
            <button className='h-12 w-full bg-blue-2 text-white rounded-3xl font-manrope font-semibold cursor-pointer border hover:opacity-75 transition duration-300 mt-6'>
              {t('change')}
            </button>
          </Link>
        </div>
