...
export default function Collaboration() {
  return (
    <>
        ...
        <p className='col-start-5 2xl:col-start-6 col-span-5 text-center mb-12'>De la selecție la livrare, vă ghidăm pas cu pas pentru a crea cadouri corporate personalizate, potrivite brandului dumneavoastră.</p>
...
const CollaborationCard = ({index, title, description}: {index: number, title: string, description: string}) => {
    return(
        <div className='rounded-3xl aspect-square col-span-3 relative flex flex-col justify-end lg:gap-3 2xl:gap-4 lg:p-6 2xl:p-8' style={{backgroundColor: `var(--blue${index})`}}>
            <span className='absolute left-8 top-12 text-white font-manrope text-8xl leading-9 font-semibold'>{index}</span>
