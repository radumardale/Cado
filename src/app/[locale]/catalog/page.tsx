...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const imageUrl =
    imagePaths[locale as keyof typeof imagePaths] || imagePaths.ro;
...
export default async function CatalogPage({
  ...
  const minMax = await queryClient.fetchQuery(
    trpc.products.getMinMaxPrice.queryOptions()
  );
  ...
  await prefetch(trpc.seasonCatalog.getSeasonCatalog.queryOptions());

  // Generate breadcrumb schema
  const baseUrl = process.env.BASE_URL || 'https://cado.md';
  const breadcrumbSchema = generateCatalogBreadcrumbSchema(baseUrl, locale, {
