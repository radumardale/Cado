...
export const addOrderRequestSchema = z
  .object({
    products: z.array(
      z.object({
        product: z.object({
  ...
  .refine(
    data => {
      if (
        !data.additional_info.billing_checkbox &&
        data.additional_info.entity_type === ClientEntity.Legal
      ) {
        if (
          !data.additional_info.billing_address.company_name ||
          data.additional_info.billing_address.company_name.length < 2
        )
          return false;
      }
  ...
  .refine(
    data => {
      if (
        ...
        if (
          !data.additional_info.billing_address.idno ||
          data.additional_info.billing_address.idno.length < 2
        )
          return false;
      }
  ...
  .refine(
    data => {
      if (
        !data.additional_info.billing_checkbox &&
        data.additional_info.entity_type === ClientEntity.Natural
      ) {
        if (
          !data.additional_info.billing_address.firstname ||
          data.additional_info.billing_address.firstname.length < 2
        )
          return false;
      }
  ...
  .refine(
    data => {
      if (
        ...
        if (
          !data.additional_info.billing_address.lastname ||
          data.additional_info.billing_address.lastname.length < 2
        )
          return false;
      }
